		...
		<view class="techan">
			<view class="techanIn">
				...
				<view class="center">
					<view class="left">
						<view class="leftIn">
							<image :src="qiniu+item.shopGoodsWithBLOBs.imageThumb" v-for="(item,index) in detail" :key="index"> </image>
...
<script>
	...
	export default {
		...
		onLoad(options) {
		   ...
		   uniRequest.get('/QianYi_Shop/selectShopOrderGoods?userId='+userId+'&shopCarId=' + this.carId + '&shopId=' + this.shopId).then(res => {
		        console.log(res)
	       
		          this.detail=res.data.data.orderEdit[0].specificationGoodsList
		          this.leng=res.data.data.orderEdit[0].specificationGoodsList.length
		...
		methods: {
			...
			dingdan: function() {
				...
				uniRequest.get('/QianYi_Shop/placeShopOrderGoods?userId=' + userId + '&goodsSpecId=' + this.guiId +
					...
					console.log(res)
					uniRequest.post('/QianYi_Shop/pay/wechat/createOrder?orderId=' + res.data.data.shopOrderId + '&type=0').then(
						res => {
							console.log(res)

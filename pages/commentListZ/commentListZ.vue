...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(ding,id)
			    uniRequest.get('/QianYi_Shop/selectShopCommentId?userId=' + userId + '&orderId=' + id).then(res => {
			      console.log(res)
			     
			
			
			        this.shopCommentsId= res.data.data.shopCommentsId
			
			
			     
			      console.log(this.shopCommentsId)
			
			    }).catch(e => {
			      console.log(e)
			    })
			uniRequest.post('/QianYi_Shop/selectOrderInfo?id=' + id).then(res => {
				console.log(res)
					...
					var temp = {
						...
						serviceScore: 5,
						 additionalEvaluation: 1,
						          ...
						          parentId: this.shopCommentsId[i]
					};
					this.com.push(temp);
				}
		...
		methods: {
			...
			submit:function(){
				...
				var that=this
				  uni.request({
				    url:  this.base+'QianYi_Shop/insertBatchShopComments?orderId=' + this.id + '&type=1',

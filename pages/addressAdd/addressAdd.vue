<template>
	<view class='whole'>
		...
		<form>
			<view class='bj'>
				<view class="contain">
					<input class='pl' placeholder='收货人' v-model="people"></input>
					<!-- <image class='lx' src='../../static/image/notebook@2x.png'></image> -->
					<input class='ph' placeholder='手机号码' v-model="phone"></input>
					...
					<textarea placeholder='详细地址：如道路、门牌号、小区、楼栋号、单元室等' v-model="detail"></textarea>
				</view>
			</view>
			<view class='bj'>
				<view class="contain">
	...
	onLoad() {
		...
		methods: {
			...
			save: function() {
				...
				uniRequest.get('/QianYi_Shop/addShopAddress?userId=' + userId + '&consigneeName=' + this.people + '&consigneePhone=' +
					this.phone + '&areaIdPath=' + this.address + '&address=' + this.detail +
					'&isDefault=' + this.kai1 + '&effectiveState=1&type=' + this.bqId).then(res => {
					console.log(res)
